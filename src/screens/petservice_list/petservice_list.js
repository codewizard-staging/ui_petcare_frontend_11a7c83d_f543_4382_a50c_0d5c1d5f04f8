...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetPetServicesMulti(query, "").then(async (resP) => {
            if (resP.status) {
                for (let i = 0; i < resP.values.length; i++) {
                    let _PetService = resP.values[i];
                    let _row = {
                        prop1: _PetService.ServiceType,
                        prop2: _PetService.Price,
                        prop3: _PetService.DurationInHours,
                        prop4: _PetService.AdvPaymentReqd,
                        prop5: _PetService.ServiceAt,
                    };
